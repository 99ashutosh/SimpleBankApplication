...
//	gtk_widget_set_sensitive(g_btn_login, FALSE);
	const char *username = gtk_entry_get_text(GTK_ENTRY(g_entry_login_username)); 
	const char *password = gtk_entry_get_text(GTK_ENTRY(g_entry_login_password));
...
G_MODULE_EXPORT void on_btn_greet_create_user_clicked(GtkWidget *wid,gpointer data){
    ...
    create_new_account(username, password, number, email);
}
